...
void PlanPath(vector<double> &next_x_vals,vector<double> &next_y_vals,double car_s,double car_d,double car_x,double car_y,double car_yaw,int prev_size,vector<double>  previous_path_x, vector<double>  previous_path_y, vector<double> &map_waypoints_x,vector<double> &map_waypoints_y,vector<double> &map_waypoints_s, vector<double>&map_waypoints_dx,vector<double> &map_waypoints_dy,int lane,double ref_vel, double safe_dist ) {
...
bool tooClose(int lane, double car_s, int prev_size, vector<vector<double>> &sensor_fusion, double safe_dist){
  ...
  for(int i = 0; i < sensor_fusion.size(); i++) {
    ...
    if (d < (2+4*lane+2) && d > (2+4*lane-2)){
      ...
      //check whether any car is too close (< safe distance)
...
int main() {
  ...
  double safe_dist = 25; // safe distance



  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&lane,&ref_vel,&safe_dist](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          ...
          if(too_close){
            ...
            if (lane_current != lane) { //if it has recommended to change to a different lane
                cout << "Car is changing to lane " << lane << std::endl;
            }
          }
          ...
          PlanPath(next_x_vals,next_y_vals,car_s,car_d,car_x,car_y,car_yaw,
                              prev_size,previous_path_x,previous_path_y,
                              map_waypoints_x,map_waypoints_y,map_waypoints_s, map_waypoints_dx,map_waypoints_dy,
                              lane,ref_vel,safe_dist );
